...
#final_data2 = final_data.filter_att(target_omic = 'DISCRETIZED', reference_omic= 'EIGENVECTOR', separator='_')
#config.save(to_save=final_data2, name="f_test_topo_integration")
...
final_data = data_utils.unpickle_objects('Topo_PARTIAL_2023-05-12-13-32-14-031004.pkl')
trained_models = data_utils.unpickle_objects('Topo_integrated_COMP_PARTIAL_models_2023-05-12-15-54-53-914194.pkl')
...
config.save(to_save=final_results, name=f"{comb_name}_final_results")
config.save(to_save=feature_importances, name=f"{comb_name}_features")
config.save(to_save=base_models, name=f"{comb_name}_base_models")
