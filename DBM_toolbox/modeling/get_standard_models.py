...
#     models['linear'] = LinearRegression()
    ...
    models['xgbslo1'] = xgb.XGBClassifier(max_depth=3, learning_rate = 0.02, n_estimators=n_trees)
    if depth > 2:
        n_trees = n_trees*2
    ...
    if depth > 1:
        ...
        models['xgbmax2'] = xgb.XGBClassifier(max_depth=5, n_estimators=n_trees, nthread=-1)
...
def get_regression_models(models=dict(), depth = 1):
    ...
    if depth < 2:
        ...
        l1ratio = [0, 0.01, 0.02, 0.05, 0.1, 0.2, 0.33, 0.5, 0.66, 0.8, 0.9, 0.95, 0.98, 0.99, 1]
    for a in alpha:
        for b in l1ratio:
    ...
    if depth > 1:
        ...
        for n1 in n1_values:
            for n2 in n2_values:
                for n3 in n3_values:
                    models['mlp'+str(n1)+'-'+str(n2)+'-'+str(n3)] = MLPRegressor(solver='sgd', learning_rate='adaptive', learning_rate_init=0.01, hidden_layer_sizes=(n1, n2, n3), verbose=False,  tol=0.00001, n_iter_no_change=nMax/2, batch_size = 32, max_iter=nMax)
    print('Defined %d models' % len(models))
    return models
...
def evaluate_models(X, y, models, X_test, folds=10, metric='accuracy'):
    ...
    for name, model in models.items():
        ...
        if scores is not None:
            ...
            print('>%s: %0.0f-val= %.3f +/- %.3f in %.3fs' % (name, folds, mean_score, std_score, timesec))
#             logging.debug('>%s: %.3f (+/- %.3f) in %.3fs' % (name, mean_score, std_score, timesec))
        else:
            print('>%s: error' % name)
    return (results, predicted)

def summarize_results(results, predicted, y_test, thisCol, maximize=True, top_n=0, graph=False):
