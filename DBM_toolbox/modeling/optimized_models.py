...
def create_XGB(**kwargs):
    return xgb.XGBClassifier(
        random_state=42, n_jobs=-1, eval_metric="logloss", **kwargs
    )
...
models = [
    ...
    {
        "name": "Logistic",
        "estimator_method": create_Logistic,
        "parameter_bounds": {
    ...
    {
        "name": "KNN",
        "estimator_method": create_KNN,
        "parameter_bounds": {"n_neighbors": ParameterBound(1, 100, discrete=True)},
    },
    {
        "name": "XGB",
        "estimator_method": create_XGB,
        "parameter_bounds": {
    ...
    {
        "name": "MLP2",
        "estimator_method": create_MLP2,
        "parameter_bounds": {  # TODO: this needs to return a tuple, not a single value?
            "hidden_layer_sizes": ParameterBound(5, 100, discrete=True),
            "alpha": ParameterBound(10e-6, 10e-2, logarithmic=True),
        },
    },
...
def retrieve_original_parameters(optimizer_parameters, parameter_bounds):
    ...
    for key in parameter_bounds.keys():
        original_parameters[key] = parameter_bounds[key].inverse_transform(
            optimizer_parameters[key]
        )
    return original_parameters


def bayes_optimize_estimator(
    estimator_method, parameter_bounds, data, targets, n_trials, metric
):
    def instantiate_cross_validate_evaluation(**kwargs):
        for parameter_bound_name in parameter_bounds:
            ...
            if parameter_bound.logarithmic:
                kwargs[parameter_bound_name] = parameter_bound.inverse_transform(
                    kwargs[parameter_bound_name]
                )
    ...
    original_parameters = retrieve_original_parameters(
        optimizer_parameters, parameter_bounds
    )
...
def bayes_optimize_models(
    data, targets, n_trials: str = None, algos: list = None, metric: str = None
):
    """
    ...
    for model in models_to_optimize:
        ...
        try:
            maximum_value, optimal_model = bayes_optimize_estimator(
                model["estimator_method"],
...
def get_standard_models(data, targets, algos: list = None, metric: str = None):
    ...
    for model in models_to_optimize:
        ...
        try:
            performance, this_model = get_estimator(
                model["estimator_method"], data, targets, metric
            )
            num = data.shape[0]
            print(performance)
        except:
