		...
		for j in np.random.randint(0, high=10000, size=reps):
			...
			for train, test in kf.split(X_shuff, y_shuff):
				...
				if models == []:
					...
					stacked_model, perf, train_perf, y_pred, y_score = stack_models(models, final_model, X_train, y_train, X_test, y_test, folds)
#				y_score = stacked_model.predict_proba(X_test)
		...
		ax.set_title('AUC in %d-times %d-fold CV: %.3f +- %.3f' % (reps, folds, mean_scores, std_scores))
	except:
		print('problem here')
