...
for n in [4]:
    ...
    final_data2 = unpickle_objects('f_testall_data_' + idx + '.pkl')

    # final_data = unpickle_objects('f_testmin_2_data_2022-08-04-13-30-00-019951.pkl')

    target_name = final_data2.dataframe.columns[-1]
    print(target_name)
    ...
    config.save(to_save=[fprs, tprs, roc_aucs], name="roc_info_bf")
    config.save(to_save=final_results, name="final_results_bf")
    config.save(to_save=feature_importances, name="features_imp_bf")
    config.save(to_save=base_models, name="base_models_bf")
